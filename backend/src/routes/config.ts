...
import { Conta, MaryData } from '../types'
...
router.post('/apr-cats', requireAuth, requirePerm('config', 'edit'), validateBody(aprCatSchema), (req: Request, res: Response): void => {
  ...
  data.aprCats.push(nome)
  writeData(data)
  audit('CONFIG_UPDATED', req, `Categoria de apreensão criada: ${nome}`)
...
router.delete('/apr-cats/:nome', requireAuth, requirePerm('config', 'edit'), (req: Request, res: Response): void => {
  ...
  writeData(data)
  audit('CONFIG_UPDATED', req, `Categoria de apreensão removida: ${nome}`)
  res.json(data.aprCats)
})
...
router.post('/restore', requireAuth, adminOnly, criticalLimiter, (req: Request, res: Response): void => {
  const body = req.body as Partial<MaryData>

  const required: (keyof MaryData)[] = ['membros', 'acoes', 'contas']
