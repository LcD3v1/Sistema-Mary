...
import relatosRoutes from './routes/relatos'
import apreensoesRoutes from './routes/apreensoes'
...
app.use('/api/apreensoes', apreensoesRoutes)
...
if (existsSync(frontendDist)) {
  ...
  console.log('[MARY] Servindo frontend de:', frontendDist)
} else {
  console.log('[MARY] Frontend dist não encontrado — rode: npm run build')
}
...
ensureDefaultAdmin().then(() => {
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[MARY] Sistema rodando em http://0.0.0.0:${PORT}`)
    console.log(`[MARY] Ambiente: ${PROD ? 'produção' : 'desenvolvimento'}`)
  })
}).catch(err => {
  console.error('[MARY] Erro ao inicializar:', err)
  process.exit(1)
})
