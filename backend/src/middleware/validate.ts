...
export const acaoSchema = z.object({
  ...
  qru:          safeStr(1, 50),                       // Tipo de Patrulha
  resultado:    z.enum(['Vitória', 'Derrota', 'Empate']).default('Vitória'),
  comandante:   z.string().max(100).optional(),
  setor:        safeStrOpt(120).default(''),
  duracao:      z.number().min(0).max(999).default(0),
  ocorrencias:  z.enum(['nenhuma', 'abordagem', 'perseguicao', 'apreensao', 'multiplas']).default('nenhuma'),
  obs:          safeStrOpt(1000).default(''),
...
export const apreensaoSchema = z.object({
  data:        z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data inválida'),
  categoria:   safeStr(1, 60),
  qtd:         z.number().int().min(1).max(100000).default(1),
  responsavel: safeStrOpt(100).default(''),
  local:       safeStrOpt(120).default(''),
  membros:     z.array(z.number().int().positive()).default([]),
  descricao:   safeStrOpt(1000).default(''),
})

export const aprCatSchema = z.object({
  nome: safeStr(1, 60),
})

export const relatoSchema = z.object({
  data:       z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data inválida'),
  relatorId:  z.number().int().positive(),
  alvoId:     z.number().int().positive(),
  nota:       z.number().min(0).max(10),
  observacao: safeStrOpt(1000).default(''),
})
