...
export function useAprCats() {
  return useQuery<string[]>({
    queryKey: ['config', 'apr-cats'],
    queryFn: async () => (await api.get<string[]>('/config/apr-cats')).data,
  })
}

export function useAddAprCat() {
  return useMutation({
    mutationFn: (nome: string) => api.post('/config/apr-cats', { nome }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['config', 'apr-cats'] }),
  })
}

export function useDeleteAprCat() {
  return useMutation({
    mutationFn: (nome: string) => api.delete(`/config/apr-cats/${encodeURIComponent(nome)}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['config', 'apr-cats'] }),
  })
}
