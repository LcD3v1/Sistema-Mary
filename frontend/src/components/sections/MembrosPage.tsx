...
interface NovoMembroForm {
  badge: string; passaporte: string; policial: string; moto: string
...
function SortableRow({ membro, index, canEdit, onUpdate, onDelete, patentes, cargos }: SortableRowProps) {
  ...
  return (
    <tr ref={setNodeRef} style={style} className="row-fade-in border-b border-bdr/50 hover:bg-bdr/30 transition-colors group">
      ...
      <td className="px-3 py-2.5">
        <InlineCell field="moto" value={membro.moto ?? ''} canEdit={canEdit} onSave={v => onUpdate(membro.id, { moto: v })} />
      </td>
      <td className="px-3 py-2.5">
    ...
    <div className="p-6 space-y-4">
              ...
              <table className="w-full text-sm min-w-[1000px]">
                <thead>
                  <tr className="border-b border-bdr">
                    {[
                      canEdit ? '⠿' : '',
                      'Badge', 'ID', 'Nome', 'Moto', 'Patente',
                      'Cargo', 'Status', 'Entrada', 'Promoção', 'Adv.',
                      canEdit ? '' : '',
                    ].map((h, i) => (
                ...
                <tbody>
                  {orderedMembros.length === 0 ? (
                    <tr>
                      <td colSpan={12} className="text-center py-12 font-mono text-xs text-txt3">
      ...
      <ModalOverlay open={modalOpen} onClose={() => setModalOpen(false)} title="NOVO MEMBRO" maxWidth="max-w-2xl">
        <form onSubmit={handleSubmit(onCreateMembro)} className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="font-mono text-xs text-txt2 tracking-wider block mb-1">ID</label>
          ...
          <div className="col-span-2">
            <label className="font-mono text-xs text-txt2 tracking-wider block mb-1">MOTO / VEÍCULO</label>
            <input {...register('moto')} placeholder="Ex: Honda CB 500" className="input-gold w-full bg-card2 border border-bdr2 rounded px-3 py-2 text-sm font-mono text-txt" />
          </div>
          <div>
          ...
          <div>
            <label className="font-mono text-xs text-txt2 tracking-wider block mb-1">CARGO</label>
