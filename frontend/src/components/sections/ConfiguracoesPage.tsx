...
export default function ConfiguracoesPage() {
  ...
  const { data: aprCats = [] } = useAprCats()
  const addAprCat = useAddAprCat()
  const deleteAprCat = useDeleteAprCat()
    ...
    <div className="p-6 space-y-4">
      ...
      <GlowCard>
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'qrus' && (
            <div>
              <h3 className="font-orbitron text-xs text-gold tracking-wider mb-1">TIPOS DE PATRULHA</h3>
              <p className="font-mono text-[11px] text-txt3 mb-4">Categorias de patrulha disponíveis</p>
              <ListEditor
                items={qrus}
          ...
          {activeTab === 'apreensao' && (
            <div>
              <h3 className="font-orbitron text-xs text-gold tracking-wider mb-1">CATEGORIAS DE APREENSÃO</h3>
              <p className="font-mono text-[11px] text-txt3 mb-4">Tipos de itens apreendidos</p>
              <ListEditor
                items={aprCats}
                onAdd={v => addAprCat.mutate(v)}
                onDelete={v => deleteAprCat.mutate(v)}
